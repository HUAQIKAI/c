...
/*因为以下四个函数定义在main函数之后*/
/*所以需要申明这四个函数*/
bool ListInsert(SqList *L, int i, int e);/*插入一个元素到顺序表*//*i表示插入位置，e表示在i位置前插入的数值*/
bool ListDelete(SqList *L, int i);/*从顺序表删除一个元素*/
int  LocateElem(SqList L, int e);/*查找一个元素在顺序表第一次出现的位置*/
void ListPrint(const SqList *L);/*打印顺序表*/
...
bool ListInsert(SqList *L, int i, int e)
{
	...
	int j;
	for (j = L->length; j >= i; j--)    /*把第i个之后的数据全部后移一位,可以理解为腾位置*/
	{
		L->data[j] = L->data[j - 1];
	}
...
bool ListDelete(SqList *L, int i)
{
	if (i >= L->length || i < 1)        /*先检查i是否为有效值*/
		return false;
	int j;
	for (j = i; j < L->length; j++)   /*开始把被删除的值后面的值前移*/
	{
		L->data[j - 1] = L->data[j];
	}
...
{
	int i;
	for (i = 0; i < L.length; i++)  /*用for循环从第一个位置开始找*/
	{
...
void ListPrint(const SqList* L) 
{
	...
	int i = 0;
	while ( (i < L->length) && (L->data[i] != (int)NULL) ) /*只要满足i小于线性表的长度且i对应的数据不为空就输出该元素*/
	{
		printf("%d\n", L->data[i]);
		i++;                             /*每次循环i自增1*/
	}
